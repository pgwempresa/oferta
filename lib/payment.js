...
async function request(path,body){
  ...
  if(!r.ok||['FAILED','REJECTED','CANCELED'].includes(d.status))throw new PaymentError(r.status===429?'Limite de requisições da operadora. Aguarde antes de tentar novamente.':'A Amplo Pay não aceitou a cobrança. Confira os dados do comprador e os detalhes no painel da operadora.',r.status===429?429:422);
  return d;
}
