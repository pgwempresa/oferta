...
const buyerDocument=require('../js/document');
...
function buildOrder(b,method,req){
  ...
  const document=buyerDocument.normalize(b.client?.document||b.document);
  if(!buyerDocument.valid(document))throw new PaymentError('Informe um CPF ou CNPJ válido do comprador.');
  client.document=document;
  const products=[{id:'sua-historinha',name:'Livro Ilustrado Personalizado',quantity:1,price:(prices.base-(b.hasDiscount===true?prices.discount:0))/100,physical:false}];
  for(const [flag,id,name,cents] of [
    ['addon_colorir','versao-colorir','Versão para Colorir',prices.colorir],
    ['addon_expressa','entrega-expressa','Prioridade: Entrega Expressa',prices.expressa],
    ['obExtras','duas-historias-extras','2 Historinhas Extras Personalizadas',prices.extras]
  ])if(b[flag]===true)products.push({id,name,quantity:1,price:cents/100,physical:false});
  const order={identifier,amount,client,products,metadata:{provider:'Sua Historinha',orderId:identifier,addon_colorir:b.addon_colorir===true,addon_expressa:b.addon_expressa===true,obExtras:b.obExtras===true}};
  if(method==='card'){
